...
final class IngestService {
    private let scraper = LinkScraperService()
    ...
    func ingestRawContent(
        ...
        if let llm = llmService, content.contains("| --- |") || content.contains("![]") {
            // 注意：由于是异步任务，我们在 ingest 之前可以尝试进行同步等待或异步处理
            // 这里为了保证即时性，采用非阻塞的逻辑，或者在 ingestURL 层面处理
        }
    ...
    private func enrichRichContent(_ content: String, llm: LLMService) async -> String {
        let prompt = """
        # Role: 知识工程专家
    ...
    /// Batch import all supported documents from a folder.
    /// - Returns: Array of created WikiPages.
    func ingestFolder(
        at url: URL,
        type: PageType = .source,
        pageStore: any AnyPageStore
    ...
    private func extractTextFromXlsx(at url: URL) -> String? {
        guard let archive = readZipArchive(at: url) else { return nil }
